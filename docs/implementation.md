...
├── main.py                      # CLI entry point — dispatches on (phase, workflow_type)
...
│   ├── transform.py             # Column combining, ID generation, schema building
...
2. Wraps each `@id` in a minimal stub of the specified `ref_type`.
...
`load_config(path)` reads a TOML file and validates it against the `Config`
discriminated union (`FlatDataConfig | ApiFetchingConfig | BiosamplesConfig |
UpliftingConfig`). The `workflow_type` key acts as the discriminator.
...
1. Define a config model in `config.py` with `workflow_type: Literal["my_workflow"]`.
2. Add it to the `Config` discriminated union.
3. Write a `run_my_workflow(config: MyWorkflowConfig) -> None` function.
4. Dispatch it in `main.py`.