...
(``id`` is mapped to ``@id``). Models are built in non-strict mode so an unknown field
...
class AddApplier:
    ...
    def apply(self, rule: AdditionRule) -> None:
        ...
        logger.info(
            ...
            rule.on_type, rule.target_property, set_count,
        )
    ...
    def _build_node(self, rule: AdditionRule, data: dict[str, Any]) -> BaseModel:
        ...
        fields = {
            ("@id" if key == "id" else key): self._build_value(rule, sub)
        ...
        if unknown:
            logger.warning(
                "Addition %s.%s: value for type %r has fields outside the schema.org "
                "model — keeping them, but they may not be intended: %s",
                rule.on_type, rule.target_property, type_name, ", ".join(unknown),
            )
        return cls(**fields)
