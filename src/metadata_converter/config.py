...
class ApiExtractorConfig(BaseModel):
    ...
    max_redirects: int = Field(
        ...
        description=(
            ...
            "Raise to ~5 only when the source uses DOI resolvers or 302-based landing-page redirects. This is meant as a safety feature."
        ),
    )
...
class CrossSheetRef(BaseModel):
    ...
    Resolved after ``@id`` generation but before schema writing, so source @ids are available.
    Used for relationships scoped to a single source file — e.g. flagged Persons → DataCatalog.
    Omit ``filter_column`` / ``filter_value`` to include all entities from ``from_sheet``.
    """

    model_config = ConfigDict(extra="forbid")
    on_sheet: str  # target sheet name (its entities receive the reference)
    property: str  # property to set on each target entity
    from_sheet: str  # source sheet name
    ref_type: str  # schema.org @type of injected references
    filter_column: str | None = None  # source column to filter on (optional)
    filter_value: Any = None  # value to match (normalized string comparison)
...
class FlatDataUpliftConfig(BaseModel):
    ...
    # @types loaded for indexing but not written to output (e.g. sample stubs).
